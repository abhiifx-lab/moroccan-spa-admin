...
export default function PaymentsPage() {
  ...
  const [sales, setSales] = useState<TraceTransaction[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [totalRevenue, setTotalRevenue] = useState(0);

  const loadSales = async () => {
    // SINGLE SOURCE OF TRUTH: All revenue transactions from Business Day Engine
    const revenueTx = await businessDayEngine.getRevenueTransactions(activeCentreFilter);
    setSales(revenueTx);
  ...
  const typeLabel = (type: string) => {
    if (type === 'booking' || type === 'booking_sale') return 'Service Sale';
    if (type === 'membership' || type === 'membership_sale') return 'Membership';
    if (type === 'gift_card' || type === 'gift_card_sale') return 'Gift Card';
    if (type === 'membership_redemption') return 'Membership Redemption';
    if (type === 'gift_card_redemption') return 'Gift Card Redemption';
