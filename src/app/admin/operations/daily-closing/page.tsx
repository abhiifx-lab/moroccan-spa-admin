...
import { businessDayEngine, DailyRegister, MonthlyRegisterMatrix, CashBookEntry, GeneralLedgerEntry, CHART_OF_ACCOUNTS } from '@/features/business-day-engine';
...
export default function AccountingEnginePage() {
  ...
  const [liveRegister, setLiveRegister] = useState<DailyRegister | null>(null);
  const [monthlyRegisterData, setMonthlyRegisterData] = useState<MonthlyRegisterMatrix | null>(null);
  ...
  const loadData = async () => {
    ...
    // 3. Cash Book (from Business Day Engine SSOT)
    const cb = await businessDayEngine.getCashBook(currentCentreObj.id, selectedDate);
    setCashBookEntries(cb);

    // 4. GL Journal (from Business Day Engine SSOT)
  ...
  const handleLockDay = async () => {
    ...
    try {
      await businessDayEngine.lockDay({
  ...
  const handleReopenDay = async (e: React.FormEvent) => {
    ...
    try {
      await businessDayEngine.reopenDay(
        currentCentreObj.id,
        selectedDate,
        reopenReason,
  ...
  const handleReverseTransaction = async (e: React.FormEvent) => {
    ...
    try {
      await businessDayEngine.reverseTransaction(
        selectedTxnForReversal.id || selectedTxnForReversal.transactionId!,
        reversalReason,
        user?.fullName || 'finance@moroccanspa.in'
      );
      alert(`✓ Transaction reversed! Counter-entry posted.`);
